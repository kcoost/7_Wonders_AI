...
class Logger:
    ...
    def log_action(self, player, action, card):
        ...
        if action == ACTION_PLAYCARD:
            ...
            text += "discarded an age %s card fto build the next wonder stage" % (
                self._get_age_string()
            )
    ...
    def log_buy_card_with_chain(self, player, card):
        text = "%s " % (player.get_name())
        text += "played %s (for free with %s)" % (
            card.pretty_print_name(),
            find_card(self.card_list, card.prechains[0]).pretty_print_name(),
        )
        self.log.append(text)
    ...
    def log_military_battle(
        self, player, player_strength, opponent, opponent_strength, score
    ):
        if player_strength > opponent_strength:
            text = (
                "%s with %d shields defeated %s with %d shields taking %d points."
                % (player, player_strength, opponent, opponent_strength, score)
            )
        elif player_strength < opponent_strength:
            text = (
                "%s with %d shields was defeated by %s with %d shields losing %d points."
                % (player, player_strength, opponent, opponent_strength, score)
            )
        else:
            text = "%s and %s both have %d strength, No points awarded." % (
                player,
                opponent,
                player_strength,
            )
